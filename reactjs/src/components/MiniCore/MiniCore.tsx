  ...
  return (
    <>
      <div className="container">
        <h1>Compute Grades Implementation</h1>
          ...
          <div className="input-center">
            {inputs.map((input, index) => (
              <div className="input-group" key={index}>
                <label htmlFor={`dateA${index}`}>Start Date:</label>
                <input
                  type="date"
                  id={`dateA${index}`}
                  value={input.dateA}
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                    handleInputChange(index, "dateA", e.target.value)
                  }
                />
                <label htmlFor={`dateB${index}`}>Final Date:</label>
                <input
                  type="date"
                  id={`dateB${index}`}
                  value={input.dateB}
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                    handleInputChange(index, "dateB", e.target.value)
                  }
                />
                <label htmlFor={`weight${index}`}>Weight (0-1):</label>
                <input
                  type="number"
                  id={`weight${index}`}
                  min="0"
                  ...
                  value={input.weight}
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                    handleInputChange(index, "weight", e.target.valueAsNumber)
                  }
                />
              </div>
            ))}
      ...
      {responseData && (
        <div className="response-container">
          ...
          <table className="response-table">
            ...
            <tbody>
              {Object.entries(responseData).map(([studentID, studentData]) => (
                <tr key={studentID}>
                  <td>{studentID}</td>
